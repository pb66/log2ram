...
ln -sf ${L2R_REPO_PATH}log2ram.service /usr/local/lib/systemd/system/log2ram.service
...
systemctl -q is-enabled log2ram || systemctl enable log2ram.service

systemctl -q is-active log2ram  && systemctl daemon-reload || echo "##### Reboot to activate log2ram #####"

 # if the first install (ie it's not already running!)
